<template>
    <div class="sidebar" :class="{'collapse': isCollapse}">
        <div class="logo">
            <img class="logo-img" src="@/icons/logo.svg" alt="昆仑银行移动发卡管理平台">
            <span class="logo-name">昆仑银行移动发卡管理平台</span>
        </div>
        <el-scrollbar wrap-class="scrollbar-wrapper" style="height:100%">
            ...
            <div v-for="(route, index) in routers" :key="index.toString()">
                <router-link :to="route.path" v-if="route.children && route.children.length === 1">
                    <el-menu-item :index="route.path" v-if="route.sidebarShow" @click="routerClick(route,index)" >
                    <i :class="route.meta.icon"></i>
                    <span slot="title">{{route.meta.title}}</span>
                    </el-menu-item>
                </router-link>
                <div v-else>
                    <el-submenu :index="index.toString()" v-if="route.sidebarShow">
                        <template slot="title" >
                            <i :class="route.meta.icon"></i>
                            <span>{{route.meta.title}}</span>
                        </template>
                        <router-link :to="item.path" v-for="(item, x) in  route.children" :key="x.toString()">
                            <el-menu-item  :index="item.path" @click="routerClick(item,x)">
...
export default {
    ...
    computed: {
        ...mapGetters([
            // 'addRouters',
            'sidebar', 'editableTabsArr', 'editableTabsValue'
        ]),
        isCollapse() {
    ...
    methods: {
        ...
        routerClick(route, index) {
            ...
            this.editableTabsArr.push(
                { title: route.meta.title, name: '' + index, path: route.path }
            )
            sessionStorage.setItem('editableTabsArr', JSON.stringify(this.editableTabsArr))
        }
    },
